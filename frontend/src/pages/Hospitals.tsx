import { hospitals as mockHospitals, computeEmergencyOccupancyPercentage } from "@/data/api";
import type { Hospital } from "@/data/api";
import { Building2, Bed, Heart, Brain, Baby, Bone, AlertTriangle, Scan, FlaskConical, Pill, Flame, Eye } from "lucide-react";

/** Sort: least occupied first (most available), then by name for consistency. */
function sortHospitalsByAvailability(hospitals: Hospital[]): Hospital[] {
  return [...hospitals].sort((a, b) => {
    const occA = computeEmergencyOccupancyPercentage(a);
    const occB = computeEmergencyOccupancyPercentage(b);
...
const serviceConfig = [
  { available: (h: Hospital) => h.traumaUnit, label: "Trauma", icon: Bone },
  { available: (h: Hospital) => h.cardiology, label: "Cardio", icon: Heart },
  { available: (h: Hospital) => h.pediatrics, label: "Peds", icon: Baby },
  { available: (h: Hospital) => h.neurosurgery, label: "Neuro", icon: Brain },
  { available: (h: Hospital) => h.radiology, label: "Radiology", icon: Scan },
  { available: (h: Hospital) => h.laboratory, label: "Lab", icon: FlaskConical },
  { available: (h: Hospital) => h.pharmacy, label: "Pharmacy", icon: Pill },
  { available: (h: Hospital) => h.burnUnit, label: "Burn Unit", icon: Flame },
  { available: (h: Hospital) => h.orthopedics, label: "Ortho", icon: Bone },
  { available: (h: Hospital) => h.ophthalmology, label: "Ophthal", icon: Eye },
] as const;

const Hospitals = () => {
  const sorted = sortHospitalsByAvailability(mockHospitals);
    ...
    <div className="h-screen flex flex-col">
      ...
      <div className="flex-1 overflow-auto p-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
          {sorted.map((h) => {
            const occupancyPct = computeEmergencyOccupancyPercentage(h);
            const isOverloaded = occupancyPct >= 90;
            const isWarning = occupancyPct >= 75;
            return (
              <div
                key={h.id}
              ...
              >
                ...
                {/* Occupancy bar — percentage from emergency beds (available/total) */}
                <div className="mb-4">
                  <div className="flex justify-between text-[10px] mb-1">
                    <span className="text-muted-foreground uppercase tracking-wider">Emergency occupancy</span>
